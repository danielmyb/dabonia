...
class Aside extends Component<AsideProps, {}> {
  render() {
    ...
    return (
      <div className="aside">
        <Link to="/">HOME</Link>
        <div className="position-absolute w-100 mb-2" style={{ bottom: "0" }}>
          ...
          <Link to={user && user.providerType === "local-userpass" ? "/logout" : "/login"} className="font-weight-bold">
            {t(user && user.providerType === "local-userpass" ? "logout" : "login")}
          </Link>
        </div>
        <nav className="mt-5">
          <ul className="list-unstyled">
